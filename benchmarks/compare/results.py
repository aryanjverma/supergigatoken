"""Best-results store and rendered table for the subword throughput benchmarks.
...
<!-- benchmarks:end --> in benchmarks/compare/SUBWORD_THROUGHPUT.md
(appending a section when the markers are absent): one collapsible table per
(cpu, dataset) with family display names, throughput per implementation, and
gigatoken's speedup vs HF/tiktoken, plus hand-curated coverage footnotes
(COVERS below, written from the `sweep.py --scan-families` data in
benchmarks/families.json). Rows without a gigatoken measurement are skipped.

These are the *inherited* subword numbers -- the same matrix upstream
gigatoken publishes, remeasured here. They live in their own document rather
than in the repo README, which is about what this fork adds (SuperBPE); the
README links here. Pass --out to send them elsewhere.
"""
...
DEFAULT_OUT = os.path.join(HERE, "SUBWORD_THROUGHPUT.md")
START, END = "<!-- benchmarks:start -->", "<!-- benchmarks:end -->"

# Header for a freshly created output document. Everything after it is
# generated, so the file carries no hand-written content beyond this.
PREAMBLE = """# Subword encoding throughput (inherited gigatoken numbers)
...
# Rendered table order (results.json sorts CPU keys alphabetically); unlisted
# CPUs sort last.
CPU_ORDER = [
...
def cmd_render(args) -> None:
    ...
    if start == -1 and end == -1:
        doc = (doc.rstrip("\n") + "\n\n" if doc.strip() else PREAMBLE) + block + "\n"
    elif start != -1 and end > start:
        doc = doc[:start] + block + doc[end + len(END):]
    else:
        raise SystemExit(f"{args.out}: malformed benchmark markers ({START} / {END})")
    with open(args.out, "w", encoding="utf-8") as f:
        f.write(doc)
    print(f"wrote {len(tables)} table(s) to {args.out}")
...
def main() -> None:
    ...
    render = sub.add_parser("render", help="rewrite the benchmark table document from the results file")
    render.add_argument("--results", default=DEFAULT_RESULTS)
    render.add_argument("--out", default=DEFAULT_OUT, help="document to rewrite the benchmark block in")
